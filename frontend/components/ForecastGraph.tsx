...
export default function ForecastGraph({ data } : { data: Prediction[] }) {
    ...
    return (
        <>
            <div className="flex flex-col lg:grid lg:grid-cols-3 w-4/5 gap-4 mt-10 mb-20 lg:mb-25">
            ...
            <div className="w-full h-[400px] lg:h-[570px] pr-3 lg:px-13">
                <ResponsiveContainer width="100%" height="100%">
                    <AreaChart data={data} width="100%" margin={{right: 15, top:3}}>
                        <CartesianGrid stroke="#374151" strokeDasharray="3 3" />
                        <XAxis dataKey="years_from_present" stroke="#ffffff" interval={0} tick={{ fontSize: 13 }} tickFormatter={(value) => `${value + new Date().getFullYear()}`} tickMargin={10}/>
                        <YAxis stroke="#ffffff" tick={{ fontSize: 13 }} tickFormatter={(value) => `£${Math.round(value / 1000)}k`} tickMargin={7}/>
